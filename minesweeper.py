...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        self.safes.add(cell)
        #3)
        new_sentence_cells = self.nearby_cells(cell) - self.safes
        new_sentence_count = count
        if len(new_sentence_cells - self.mines) < len(new_sentence_cells):
            new_sentence_count = new_sentence_count - len(new_sentence_cells) + len(new_sentence_cells - self.mines)
            new_sentence_cells = new_sentence_cells - self.mines
        new_sentence_cells = new_sentence_cells - self.safes
        new_sentence = Sentence(new_sentence_cells, new_sentence_count)
        self.knowledge.append(new_sentence)
        #OLD #3 - self.knowledge.append(Sentence(self.nearby_cells(cell) - self.safes,count))
        #4-5)
        new_sentence_length = len(new_sentence.cells)
        new_knowledge = []
        for sentence in self.knowledge:
